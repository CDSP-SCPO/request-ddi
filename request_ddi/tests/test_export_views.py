...
class ExportQuestionsCSVViewTest(TestCase):
    ...
    def test_export_single_year(self):
        ...
        response = self.client.get(reverse("export_questions_csv"), {"years": ["2022"]})
        self.assertEqual(response.status_code, 200)
        content = response.content.decode("utf-8")
        reader = csv.reader(StringIO(content))
        rows = list(reader)
        ...
        self.assertNotIn(
            "Q3", dataset_vars_str
        )  # Q3 est un binding sur 2024, mais il est un binding de la même variable

    def test_export_multiple_years(self):
        """Teste l'export CSV avec plusieurs années (2022 et 2024)."""
        response = self.client.get(reverse("export_questions_csv"), {"years": ["2022,2024"]})
        self.assertEqual(response.status_code, 200)
        content = response.content.decode("utf-8")
        reader = csv.reader(StringIO(content))
        rows = list(reader)
