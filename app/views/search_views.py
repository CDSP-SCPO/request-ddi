...
class SearchResultsDataView(ListView):
    ...
    def build_filtered_search(self):
        ...
        survey_filter = [
            int(survey_id) for survey_id in survey_filter if survey_id.isdigit()
        ]
        ...
        collections_filter = [
            int(collection_id)
            for collection_id in collections_filter
        ...
        if survey_filter:
            ...
            search = search.filter(
                "terms", **{"survey.subcollection.id": subcollection_filter}
            )
