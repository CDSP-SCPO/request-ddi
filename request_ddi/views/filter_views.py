...
def get_surveys_by_collections(request):
    ...
    if collections_ids:
        ...
        surveys = Survey.objects.filter(subcollection__collection__id__in=collections_ids).order_by(
            "name"
        )
    else:
...
def get_subcollections_by_collections(request):
    ...
    if not collection_ids:
        ...
        subcollections = Subcollection.objects.filter(collection_id__in=collection_ids).order_by(
            "name"
        )
        surveys = Survey.objects.filter(subcollection__collection_id__in=collection_ids).order_by(
            "name"
        )
