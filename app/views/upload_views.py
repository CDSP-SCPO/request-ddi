...
# -- LOCAL
from app.forms import CSVUploadFormCollection, XMLUploadForm
...
class XMLUploadView(FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            num_records, num_new_variables, num_new_bindings = importer.import_data(
                question_datas
            )
    ...
    def form_invalid(self, form):
        ...
        if error_messages:
            messages.error(
                self.request, "<br/>".join(error_messages), extra_tags="safe"
            )
...
def check_duplicates(request):
    if request.method == "POST":
        ...
        if not id_tag or not id_tag.text.strip():
            return JsonResponse(
                {"error": "IDNo manquant dans le fichier XML"}, status=400
            )
        ...
        if existing_variables:
            return JsonResponse(
                {"status": "exists", "existing_variables": existing_variables}
            )
        else:
