...
from request_ddi.core.models import Collection, Subcollection, Survey
...
class JSONViewsTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.survey = Survey.objects.create(
            name="Survey Test", subcollection=cls.subcollection, start_date="2020-01-01"
        )
    ...
    def test_get_subcollections_by_collections(self):
        ...
        response = self.client.get(
            reverse("request_ddi_api:get_subcollections_by_collections"),
    ...
    def test_get_decades(self):
        ...
        response = self.client.get(
            reverse("request_ddi_api:get_decades"),
            {"collections_ids": str(self.collection.id)},
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_get_years_by_decade(self):
        ...
        response = self.client.get(
            reverse("request_ddi_api:get_years_by_decade"),
            {
                "decade": "2020",
                "collections_ids": str(self.collection.id),
            },
