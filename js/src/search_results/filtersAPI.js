...
import { attachDynamicCheckboxEvents } from "./events.js";

export async function updateSubcollections(selectedCollections) {
  const idsParam = selectedCollections?.length ? selectedCollections.join(",") : "";

  const response = await fetch(
    `/api/${window.requestDdiData.apiVersion}/get-subcollections-by-collections/?collections_ids=${idsParam}`
  );
...
export async function updateSurveys(selectedSubcollections) {
  const idsParam = selectedSubcollections?.length ? selectedSubcollections.join(",") : "";

  const response = await fetch(
    `/api/${window.requestDdiData.apiVersion}/get-surveys-by-subcollections/?subcollections_ids=${idsParam}`
  );
